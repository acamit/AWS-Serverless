...
```bash
step-functions$ sam build --use-container
```
...
```bash
step-functions$ sam logs -n StockCheckerFunction --stack-name "step-functions" --tail
```
...
```bash
step-functions$ pip install -r tests/requirements.txt --user
# unit test
step-functions$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
step-functions$ AWS_SAM_STACK_NAME="step-functions" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "step-functions"
```
